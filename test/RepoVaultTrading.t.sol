...
//        usdc_spoof.mint(address(this), 1000);
        ...
        usdc_spoof.mint(lp, 100 * DECIMALS);
    ...
    function test_go_long() public {
        ...
        usdc.approve(address(pool), 100 * DECIMALS);
        pool.goLong(100 * 10**6, 1 days, 99 * 10 ** 6);
        vm.stopPrank();
        console.log("balance: ", usdc.balanceOf(UserWallet));
        assertGe(aUSDCPT.balanceOf(address(pool)), 8 * 10 ** 5);
        console.log("PT Balance: ", aUSDCPT.balanceOf(address(pool)));
